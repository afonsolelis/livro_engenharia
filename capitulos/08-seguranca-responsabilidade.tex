...
O engenheiro de software funciona como a primeira linha de defesa de qualquer
...
O custo real de falhas de segurança não é abstrato. Não estamos falando de
...
Em dezembro de 2021, a comunidade de engenharia de software foi sacudida pela
...
eram há meio século. Entender esses princípios a fundo é o que separa o
...
Zero Trust se manifesta, concretamente, em quatro pilares. O primeiro
...
padrão para falha é 403 (Forbidden), não 200 (OK). O oposto, o \textbf{fail
...
automaticamente por padrão, mais um exemplo de secure by default. O
...
atacante teste milhões de combinações de credenciais sem ser bloqueado. A
ausência de autenticação multi-fator (MFA) significa que uma senha comprometida,
seja via phishing, via reutilização ou via breach de outro serviço, é suficiente
...
desse problema, mas o risco se estende a dependências transitivas, ou seja,
bibliotecas que você nunca importou diretamente mas que foram trazidas por suas
dependências diretas. O Log4Shell demonstrou esse risco de forma brutal: muitas
equipes descobriram que usavam Log4j não porque a incluíram explicitamente, mas
porque ela era uma dependência transitiva de frameworks como Spring Boot.
...
Esta é a vulnerabilidade silenciosa, aquela que permite que todas as outras passem
...
escreveram código sem assistência. O aspecto mais preocupante não era a
qualidade do código em si, e sim a \emph{confiança} dos desenvolvedores. Aqueles
...
verdade, deve receber \emph{mais} escrutínio, justamente porque a confiança
...
desenvolvimento de software. Até aí, nada incomum, pois milhares de empresas
...
antes de usá-los em decisões críticas, já que o LLM pode alucinar dados, inventar
...
checksums. Funciona como a lista de ingredientes de um alimento industrializado,
...
SBOMs devem ser gerados automaticamente como parte do pipeline de
...
A verificação de dependências opera em três pilares que, juntos,
...
atacante obtém acesso ao storage subjacente, seja um disco roubado, um backup
exfiltrado ou um snapshot de banco copiado. Serviços de cloud como AWS RDS, Azure
...
dados em ambientes de cloud onde o provedor de infraestrutura é, no fim das
contas, um terceiro em quem se prefere não confiar cegamente.
...
de forma automática, não manual. Se uma chave é comprometida mas não é
...
corretamente anonimizados não são mais considerados dados pessoais e, portanto,
não estão sujeitos à LGPD ou ao GDPR. Mas a anonimização real é
...
cinematográficas, o que potencialmente revelava informações sensíveis sobre
...
contas foram comprometidas antes do rate limiting. Ao todo, 847 contas tiveram login
...
critérios predefinidos e determinar o escopo, ou seja, quais sistemas, quais dados,
...
\textbf{Erradicação}: remover a causa raiz do incidente, o que inclui aplicar patches,
...
a tentação de agir impulsivamente é quase irresistível. Nesses momentos a
disciplina do engenheiro é mais testada e mais necessária.
...
Saiba pedir ajuda. Se você não sabe resolver, escale. Chame colegas mais
...
Prepare-se para o incidente. Não é ``se''; é ``quando''. A diferença entre
