...
ao banco de dados, e o sistema ``funciona'' em demonstrações. No entanto, por
...
Fowler chama essas de ``decisões que são difíceis de reverter'', e é precisamente
essa irreversibilidade que justifica o investimento de tempo em reflexão
...
você terá uma visão integrada de como essas peças se encaixam na prática.
...
escolher o padrão certo para o contexto certo, e sabe explicar o porquê. E,
talvez mais importante, é aquele que sabe quando \textit{não} escolher um
padrão sofisticado, quando a simplicidade é a melhor arquitetura possível.
...
A ideia foi articulada pela primeira vez por Edsger Dijkstra em 1974, em um
artigo seminal onde ele argumentou que a única forma de lidar com a complexidade
...
--- estava falando de uma limitação cognitiva humana. O cérebro humano consegue
...
coeso é compreensível porque tudo dentro dele ``faz sentido junto'' --- não há
surpresas, não há funcionalidades escondidas. Um módulo com baixo acoplamento é
...
baixo nível; ambos devem depender de abstrações. Na prática, isso significa que
a lógica de negócio de um sistema de pagamentos não deve depender diretamente
do PostgreSQL --- deve depender de uma abstração de ``repositório de transações''
...
A Lei de Demeter foi formulada em 1987 na Northeastern University, durante o
...
--- qualquer reestruturação interna desses objetos quebra o código que depende
da cadeia.
...
Antes de mergulhar nos detalhes, vale uma observação importante: estilos
...
Nos primeiros décadas da computação, todo software era monolítico --- não por
...
monolito modular degenera em um ``big ball of mud''. E há um limite prático
...
protocolos simples como HTTP/REST, e com governança descentralizada --- cada
...
monolito distribuído --- foi cunhado precisamente para descrever essa situação
infeliz.
...
A ideia de sistemas orientados a eventos tem raízes que remontam aos anos 1980,
com os primeiros sistemas de publish-subscribe. No entanto, o estilo ganhou
...
publica ``PedidoFoiCriado'', ele não sabe e não se importa se o serviço de
...
profunda: times podem trabalhar de forma independente, cada um publicando e
...
É importante notar que esta tabela é uma simplificação deliberada. Cada célula
...
A evolução arquitetural não é um salto --- é uma jornada. Nenhum sistema nasce
...
reclama que o mapa-múndi não mostra as ruas de São Paulo --- cada nível de
...
É importante notar que ``container'' aqui não se refere a Docker containers,
...
comunicar suas decisões de forma clara para audiências diferentes não é um
bom arquiteto --- é apenas alguém que sabe muitas coisas mas não consegue
transmiti-las. E conhecimento não transmitido é conhecimento perdido.

O domínio da comunicação arquitetural por meio do C4 nos prepara para o
próximo passo: entender profundamente o domínio de negócio que a arquitetura
...
A premissa fundamental do DDD é que o maior risco em projetos de software não
...
Essa prática parece simples, quase trivial, mas sua importância é profunda.
...
Para entender a diferença na prática, considere uma nota de R\$~50. A nota
...
mesma porta usando um dicionário Python --- perfeito para testes. Todos
...
boa não é aquela que previu todas as mudanças --- é aquela que acomoda
...
Além de verificações de dependência, fitness functions podem monitorar diversos
aspectos da saúde arquitetural. O tempo de resposta dos endpoints da API pode
...
``restrições que o sistema impõe''. A diferença é profunda: recomendações
...
O nome do padrão foi cunhado por Martin Fowler em 2004, inspirado por
...
O padrão Strangler Fig nos ensina algo profundo sobre a natureza da mudança
...
na prática, cada um contribuindo para decisões que se reforçam mutuamente.
...
de KYC --- Know Your Customer --- exigido por regulamentação financeira), que leva
...
proposta. Porém, é essencial entender seus limites para usá-la de forma
eficaz e segura.
